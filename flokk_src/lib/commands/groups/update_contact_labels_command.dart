...
class UpdateContactLabelsCommand extends AbstractCommand
  ...
  Future<ContactData?> execute(ContactData contact) async {
    ...
    await executeAuthServiceCmd(() async {
      ...
      List<GroupData> existingGroups =
          contactsModel.getContactById(contact.id)?.groupList ?? [];

      //The updated labels for contact
      List<GroupData> updatedGroups = contact.groupList;

      List<GroupData> removeFrom = existingGroups
          .where((x) => !updatedGroups.any((y) => y.id == x.id))
          .toList();
      List<GroupData> addTo = updatedGroups
          .where((x) => !existingGroups.any((y) => y.id == x.id))
          .toList();
      ...
      for (var n in removeFrom) {
        result = await googleRestService.groups
            .modify(authModel.googleAccessToken, n, removeContacts: [contact]);
      ...
      for (var n in addTo) {
        result = await googleRestService.groups
            .modify(authModel.googleAccessToken, n, addContacts: [contact]);

...
class CreateLabelCommand extends AbstractCommand
  ...
  Future<GroupData?> execute(String labelName) async {
    ...
    ServiceResult<GroupData> result = await executeAuthServiceCmd(() async {
      ServiceResult<GroupData> result = await googleRestService.groups
          .create(authModel.googleAccessToken, newGroup);

      if (result.success) contactsModel.allGroups.add(result.content!);
